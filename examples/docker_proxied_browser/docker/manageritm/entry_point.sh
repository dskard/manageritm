...
if [ x"$USER_ID" != x"0" -a x"$USER_ID" != x"1000" ]; then
    ...
    cat /etc/passwd | sed -e "s/^manageritm:x:1000:1000:/manageritm:x:$USER_ID:$GROUP_ID:/" > $NSS_WRAPPER_PASSWD
    cat /etc/group | sed -e "s/^manageritm:x:1000:/manageritm:x:$GROUP_ID:/" > $NSS_WRAPPER_GROUP
